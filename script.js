window.onload = () => {
	...
	mainText.addEventListener('blur', () => {
		if (mainText.value != '') mainText.focus();
	});
	...
	addButton.addEventListener('click', () => {
		...
		if (!(addTaskValue.trim().length === 0)) {
			const text_Area = myFunctions.crtElements(
				'li',
				['tasksArea'],
				taskAreaAll
			);
			const customChk = myFunctions.crtElementsWithAttributes(
				'span',
				['customCheckBox'],
				'data-buttonType',
				'notChecked',
			...
			const edtDlt_Btns = myFunctions.crtElements(
				'div',
				['edtDltBtns'],
				text_Area
			);
			const edt = myFunctions.crtElementsWithAttributes(
				'i',
			...
			const dlt = myFunctions.crtElements(
				'i',
			...
			const current_Time = myFunctions.crtElements(
				'input',
				['currentTime'],
				text_Area
			);
			current_Time.setAttribute('readonly', true);
			current_Time.value = `${dateNow.toLocaleString('en-US', {
				hour: 'numeric',
				minute: 'numeric',
				second: 'numeric',
				hour12: true,
			...
			edt.addEventListener('click', () => {
				if (edt.getAttribute('data-buttonType') === 'edit') {
					...
					if (!(txt.value.trim().length === 0)) {
						...
						edt.classList.remove('fa-floppy-disk');
						edt.classList.add('fa-pen-to-square');
						txt.style.cursor = 'grab';
					} else myFunctions.alertFunction('Cannot Save an Empty Task!');
				}
			});
			...
			customChk.addEventListener('click', () => {
				...
				} else myFunctions.alertFunction('Cannot Mark Empty Task As Done!');
			});
		...
		} else myFunctions.alertFunction('Cannot Make An Empty Task!');
	});
};
...
const myFunctions = {
	...
	crtElementsWithAttributes(
		...
		const createdElement = this.crtElements(
			crtElementTag,
			[...appendingClass],
			appendTo
		);
